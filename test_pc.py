...
def test_compare_cards_equal(low):
    ...
    # 1 card equal and 2 lower
    assert compare_cards(hand1, 2) == low[2]
    # 2 cards equal and one lower
    assert compare_cards(hand2, 2) == {
        "number": "5",
        "type": "coin",
        "truco": "2",
        "envido": "5",
    } or {"number": "5", "type": "sword", "truco": "2", "envido": "5"}
...
def test_choose_card_secondPhase(low, medium, high):
    ...
    hand1, hand2, hand3 = medium[:2], high[:2], low[:2]
    # First to play
...
def test_add_weight(list):
    ...
    assert add_weight(list, 0.35, 0.40, 0.25) == {
        "accept": 0.35,
        "reject": 0.40,
        "reply": 0.25,
    }
    assert add_weight(list, 0.35, 0.40, 0.25, "yes") == {
        "accept": 0.35,
        "reject": 0.40,
        "reply": 0.25,
    }
...
def test_assign_weight_firstRow(list):
    ...
    assert assign_weight(1, ["PC"], list, ["t1", "t1", "t2"]) == {
        "accept": 0.07,
        "reject": 0.90,
        "reply": 0.03,
    }
    assert assign_weight(1, ["PC"], list, ["t2", "t2", "t2"]) == {
        "accept": 0.30,
        "reject": 0.60,
        "reply": 0.10,
    }
    assert assign_weight(1, ["PC"], list, ["t1", "t1", "t3"]) == {
        "accept": 0.15,
        "reject": 0.75,
        "reply": 0.10,
    }
    assert assign_weight(1, ["PC"], list, ["t2", "t2", "t3"]) == {
        "accept": 0.60,
        "reject": 0.20,
        "reply": 0.20,
    }
    assert assign_weight(1, ["PC"], list, ["t3", "t3", "t3"]) == {
        "accept": 0.40,
        "reject": 0.00,
        "reply": 0.60,
    }
    # Test calls
    list = ["True", "False"]
    assert assign_weight(1, [""], list, ["t1", "t2", "t3"]) == {
        "True": 0.55,
        "False": 0.45,
    }
    assert assign_weight(1, [""], list, ["t3", "t3", "t3"]) == {
        "True": 1.00,
        "False": 0.00,
    }
...
def test_assign_weight_secondRow(list):
    ...
    assert assign_weight(2, ["PC"], list, ["t2", "t2"]) == {
        "accept": 0.50,
        "reject": 0.35,
        "reply": 0.15,
    }
    assert assign_weight(2, ["PC"], list, ["t1", "t3"]) == {
        "accept": 0.70,
        "reject": 0.15,
        "reply": 0.15,
    }
    assert assign_weight(2, ["tie"], list, ["t1", "t1"]) == {
        "accept": 0.10,
        "reject": 0.70,
        "reply": 0.20,
    }
    assert assign_weight(2, ["tie"], list, ["t2", "t2"]) == {
        "accept": 0.25,
        "reject": 0.50,
        "reply": 0.25,
    }
    assert assign_weight(2, ["tie"], list, ["t1", "t3"]) == {
        "accept": 0.40,
        "reject": 0.30,
        "reply": 0.30,
    }
    assert assign_weight(2, ["emi"], list, ["t1", "t1"]) == {
        "accept": 0.05,
        "reject": 0.90,
        "reply": 0.05,
    }
    assert assign_weight(2, ["emi"], list, ["t1", "t2"]) == {
        "accept": 0.10,
        "reject": 0.83,
        "reply": 0.07,
    }
    assert assign_weight(2, ["emi"], list, ["t2", "t3"]) == {
        "accept": 0.45,
        "reject": 0.20,
        "reply": 0.35,
    }
    ...
    assert assign_weight(2, ["tie"], list, ["t1", "t2"]) == {
        "True": 0.15,
        "False": 0.85,
    }
    assert assign_weight(2, ["emi"], list, ["t3", "t3"]) == {
        "True": 0.95,
        "False": 0.05,
    }
...
def test_assign_weight_thirdRow(list):
    ...
    assert assign_weight(3, ["PC"], list, ["t2"]) == {
        "accept": 0.20,
        "reject": 0.20,
        "reply": 0.60,
    }
    assert assign_weight(3, ["PC"], list, ["t3"]) == {
        "accept": 0.09,
        "reject": 0.01,
        "reply": 0.90,
    }
    assert assign_weight(3, ["tie"], list, ["t1"]) == {
        "accept": 0.05,
        "reject": 0.50,
        "reply": 0.45,
    }
    assert assign_weight(3, ["tie"], list, ["t2"]) == {
        "accept": 0.20,
        "reject": 0.20,
        "reply": 0.60,
    }
    assert assign_weight(3, ["tie"], list, ["t3"]) == {
        "accept": 0.10,
        "reject": 0.05,
        "reply": 0.85,
    }
    assert assign_weight(3, ["emi"], list, ["t1"]) == {
        "accept": 0.05,
        "reject": 0.90,
        "reply": 0.05,
    }
    assert assign_weight(3, ["emi"], list, ["t2"]) == {
        "accept": 0.20,
        "reject": 0.50,
        "reply": 0.30,
    }
    assert assign_weight(3, ["emi"], list, ["t3"]) == {
        "accept": 0.45,
        "reject": 0.10,
        "reply": 0.45,
    }
...
def test_add_envido_weight(list, e_list):
    with pytest.raises(ValueError, match="Weight sum is not 1"):
        add_envido_weight(e_list, 0.30, 0.69)
    assert add_envido_weight(e_list, 0.15, 0.25, 0.60) == {
        "envido": 0.15,
        "real envido": 0.25,
        "falta envido": 0.60,
    }
    e_list.pop(0)
    with pytest.raises(ValueError, match="Weight sum is not 1"):
        add_envido_weight(e_list, 0.70, 0.40)
    assert add_envido_weight(e_list, 0.70, 0.30) == {
        "real envido": 0.70,
        "falta envido": 0.30,
    }
    assert add_envido_weight(list, 0.10, 0.85, 0.05) == {
        "accept": 0.10,
        "reject": 0.85,
        "reply": 0.05,
    }
    list.pop()
    assert add_envido_weight(list, 0.30, 0.70) == {
        "accept": 0.30,
        "reject": 0.70,
    }
...
def test_assign_envido_weight(list):
    assert assign_envido_weight(list, "q1", 0, 0) == {
        "accept": 0.10,
        "reject": 0.85,
        "reply": 0.05,
    }
    assert assign_envido_weight(list, "q3", 0, 1) == {
        "accept": 0.50,
        "reject": 0.25,
        "reply": 0.25,
    }
    assert assign_envido_weight(list, "q1", 2, 0) == {
        "accept": 0.05,
        "reject": 0.85,
        "reply": 0.10,
    }
    assert assign_envido_weight(list, "q2", 2, 0) == {
        "accept": 0.20,
        "reject": 0.60,
        "reply": 0.20,
    }
    assert assign_envido_weight(list, "q3", 4, 0) == {
        "accept": 0.55,
        "reject": 0.10,
        "reply": 0.35,
    }
    assert assign_envido_weight(list, "q4", 4, 0) == {
        "accept": 0.45,
        "reject": 0.01,
        "reply": 0.54,
    }
...
def test_assign_envido_reply_weight(e_list):
    assert assign_envido_reply_weight(e_list, "q1", 0) == {
        "envido": 0.60,
        "real envido": 0.30,
        "falta envido": 0.10,
    }
    assert assign_envido_reply_weight(e_list, "q4", 0) == {
        "envido": 0.15,
        "real envido": 0.25,
        "falta envido": 0.60,
    }
    e_list.pop(0)
    assert assign_envido_reply_weight(e_list, "q2", 1) == {
        "real envido": 0.60,
        "falta envido": 0.40,
    }
    assert assign_envido_reply_weight(e_list, "q3", 1) == {
        "real envido": 0.50,
        "falta envido": 0.50,
    }
